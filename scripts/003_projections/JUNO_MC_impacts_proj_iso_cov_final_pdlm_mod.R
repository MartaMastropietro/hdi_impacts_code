...
# cov
cov_table<-read_csv(file.path(coefs_dir,paste0("gr_",out_var, '_',type, '_',spec,'_lagN',as.character(N),'_cov_iso.csv')))
cov_table$...1<-NULL
...
# Set very uncertain coefficients to zero
all(names(coefs) == rownames(cov_table))
all(rownames(cov_table) == colnames(cov_table))

# set controls
if(out_var=="eys"){
  
  drop_idx <- grep("diff_TM|diff_RR", names(coefs))
  keep_idx <- setdiff(seq_along(coefs), drop_idx)
  coefs <- coefs[keep_idx]
  cov_table <- cov_table[keep_idx, keep_idx]
...
if(out_var=="gnipc"){
  ...
  drop_idx <- grep("diff_RR", names(coefs))
  keep_idx <- setdiff(seq_along(coefs), drop_idx)
  coefs <- coefs[keep_idx]
  cov_table <- cov_table[keep_idx, keep_idx]
...
arrow::write_feather(data_all,file.path(out_dir_lag,
                                        paste0("gr_",out_var,'_',spec,'_',type,'_',  agg, "_nlags",N,
                                               "_boot_impacts_iso_cov_intervals" , ".feather")) )
