...
out_dir<-file.path(out_dir, paste0(rob, "_cov"))
if(!dir.exists(out_dir)){dir.create(out_dir)}
...
out_var<-"hdi"
glob_data<-read_csv(file.path(out_dir_lag,paste0('c_sel_agg_',out_var,"_",type,'_',spec, "_pop_weight_nlags_",N,"_",rob,"_cov_boot_all.csv")))
...
glob_data$delta_hdi_q05<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant05)
glob_data$delta_hdi_q95<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant95)
...
glob_data$perc_delta_hdi_q05<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant05)
glob_data$perc_delta_hdi_q95<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant95)
...
glob_data$hdi_cc_q05<-apply(glob_data%>%select(matches("^hdi_cc\\.\\d+$")), 1, quant05)
glob_data$hdi_cc_q95<-apply(glob_data%>%select(matches("^hdi_cc\\.\\d+$")), 1, quant95)

all_data<-glob_data[, c("ssp", "model", "iso3", "year",  "hdi",
                        "delta_hdi_mean", "delta_hdi_med","delta_hdi_q05", "delta_hdi_q95",
                        "perc_delta_hdi_mean", "perc_delta_hdi_med","perc_delta_hdi_q05", "perc_delta_hdi_q95",
                        "hdi_cc_mean", "hdi_cc_med","hdi_cc_q05", "hdi_cc_q95")]
...
glob_data$delta_income_index_q05<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant05)
glob_data$delta_income_index_q95<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant95)
...
glob_data$perc_delta_income_index_q05<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant05)
glob_data$perc_delta_income_index_q95<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant95)
...
glob_data$income_index_cc_q05<-apply(glob_data%>%select(matches("^income_index_cc\\.\\d+$")), 1, quant05)
glob_data$income_index_cc_q95<-apply(glob_data%>%select(matches("^income_index_cc\\.\\d+$")), 1, quant95)

all_data<-glob_data[, c("ssp", "model", "iso3", "year",  "income_index",
                        "delta_income_index_mean", "delta_income_index_med","delta_income_index_q05", "delta_income_index_q95",
                        "perc_delta_income_index_mean", "perc_delta_income_index_med","perc_delta_income_index_q05", "perc_delta_income_index_q95",
                        "income_index_cc_mean", "income_index_cc_med","income_index_cc_q05", "income_index_cc_q95")]
# write.csv(x=all_data, file=file.path(out_dir_lag,paste0('c_sel_d_s_agg_',out_var,"_",type,'_',spec,'_', "_pop_weight_nlags_",N,"boot_int.csv")), row.names = FALSE)
...
glob_data$delta_lifex_index_q05<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant05)
glob_data$delta_lifex_index_q95<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant95)
...
glob_data$perc_delta_lifex_index_q05<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant05)
glob_data$perc_delta_lifex_index_q95<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant95)
...
glob_data$lifex_index_cc_q05<-apply(glob_data%>%select(matches("^lifex_index_cc\\.\\d+$")), 1, quant05)
glob_data$lifex_index_cc_q95<-apply(glob_data%>%select(matches("^lifex_index_cc\\.\\d+$")), 1, quant95)

all_data<-glob_data[, c("ssp", "model", "iso3", "year",  "lifex_index",
                        "delta_lifex_index_mean", "delta_lifex_index_med","delta_lifex_index_q05", "delta_lifex_index_q95",
                        "perc_delta_lifex_index_mean", "perc_delta_lifex_index_med","perc_delta_lifex_index_q05", "perc_delta_lifex_index_q95",
                        "lifex_index_cc_mean", "lifex_index_cc_med","lifex_index_cc_q05", "lifex_index_cc_q95")]
# write.csv(x=all_data, file=file.path(out_dir_lag,paste0('c_sel_d_s_agg_',out_var,"_",type,'_',spec,'_', "_pop_weight_nlags_",N,"boot_int.csv")), row.names = FALSE)
data<-inner_join(data, all_data)
...
glob_data$delta_edu_index_q05<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant05)
glob_data$delta_edu_index_q95<-apply(glob_data%>%select(matches("^delta\\.\\d+$")), 1, quant95)
...
glob_data$perc_delta_edu_index_q05<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant05)
glob_data$perc_delta_edu_index_q95<-apply(glob_data%>%select(matches("^perc_delta\\.\\d+$")), 1, quant95)
...
glob_data$edu_index_cc_q05<-apply(glob_data%>%select(matches("^edu_index_cc\\.\\d+$")), 1, quant05)
glob_data$edu_index_cc_q95<-apply(glob_data%>%select(matches("^edu_index_cc\\.\\d+$")), 1, quant95)

all_data<-glob_data[, c("ssp", "model", "iso3", "year",  "edu_index",
                        "delta_edu_index_mean", "delta_edu_index_med","delta_edu_index_q05", "delta_edu_index_q95",
                        "perc_delta_edu_index_mean", "perc_delta_edu_index_med","perc_delta_edu_index_q05", "perc_delta_edu_index_q95",
                        "edu_index_cc_mean", "edu_index_cc_med","edu_index_cc_q05", "edu_index_cc_q95")]
# write.csv(x=all_data, file=file.path(out_dir_lag,paste0('c_sel_d_s_agg_',out_var,"_",type,'_',spec,'_', "_pop_weight_nlags_",N,"boot_int.csv")), row.names = FALSE)
data<-inner_join(data, all_data)

write.csv(x=data, file=file.path(out_dir_lag,paste0('c_sel_agg_all_idx_',type,'_',spec,'_', "_pop_weight_nlags_",N,"_",rob,"_cov_boot_all.csv")), row.names = FALSE)
...
write.csv(x=data, file=file.path(out_dir_lag,paste0('c_sel_med_agg_all_idx_',type,'_',spec,'_', "_pop_weight_nlags_",N,"_",rob,"_cov_boot_int.csv")), row.names = FALSE)
  ...
  for (sc in unique(data$ssp)){
    ...
    long_data <- filtered_data %>%
      select(iso3, model, 
             ...
             delta_hdi_q05, delta_income_index_q05, delta_edu_index_q05, delta_lifex_index_q05,
             delta_hdi_q95, delta_income_index_q95, delta_edu_index_q95, delta_lifex_index_q95) %>%
    ...
    long_data <- filtered_data %>%
      select(iso3, model, 
             ...
             perc_delta_hdi_q05, perc_delta_income_index_q05, perc_delta_edu_index_q05, perc_delta_lifex_index_q05,
             perc_delta_hdi_q95, perc_delta_income_index_q95, perc_delta_edu_index_q95, perc_delta_lifex_index_q95) %>%
      pivot_longer(cols = -c(iso3, model), 
                   names_to = c("variable", "stat"), 
...
data<-read_csv(file.path(out_dir_lag,paste0('c_sel_med_agg_all_idx_',type,'_',spec,'_', "_pop_weight_nlags_",N,"_",rob,"_cov_boot_int.csv")))
...
sel_countries<-c("USA",  "ITA", "RUS", "CHN", "VEN", "ETH",  "AFG", "IND")
...
variables <- c("income_index", "edu_index", "lifex_index", "hdi")

var_labels <- c(
  "income_index" = "Income Index",
  "edu_index"    = "Education Index",
  "lifex_index"  = "Life Expectancy Index",
  "hdi"          = "HDI"
)
...
for (y in c(2030, 2050, 2080, 2100)) {
  ...
  filtered_data <- data %>%
    filter(year == y, iso3 %in% sel_countries) %>%
    mutate(iso3 = factor(iso3, levels = sel_countries))
  ...
  p_abs <- ggplot(long_data,
                  aes(x = iso3, y = 100 * mean, color = ssp)) +
    geom_errorbar(aes(ymin = 100 * q10, ymax = 100 * q90),
    ...
    theme(
      ...
      text = element_text(family = "CM Roman", size = 18)
    ) +
    scale_color_brewer(palette = "Set1")
  
  ggsave(
    file.path(out_dir_sel, paste0("all_variables_abs_", y, "_", type, "_", spec, ".png")),
    p_abs, width = 12, height = 5, dpi=400
  )
  ...
  p_perc <- ggplot(long_data,
    ...
    facet_wrap(~ variable, scales = "free_y", ncol = 2) +
    labs(
      # title = paste("Percentage Index Changes -", y),
      x = "Country",
      y = "Change (%)",
      color = "SSP"
    ) +
  ...
  ggsave(
    file.path(out_dir_sel, paste0("all_variables_perc_", y, "_", type, "_", spec, ".png")),
    p_perc, width = 12, height = 5, dpi=400
  )
}
...
for (y in c(2030, 2050, 2080, 2100)){
  ...
  g<-ggplot(long_data, aes(x = variable, y = 100 * mean, color = ssp)) +
    ...
    geom_abline(slope=0, intercept=0)+
    scale_x_discrete(limits = variable_order) + guides(y = "axis_logticks")+
...
for (sc in unique(data$ssp)){
  ...
  g<-ggplot(data_comp, aes(x = 100*delta_lifex_index_mean, y = 100*delta_income_index_mean, color = 100*delta_edu_index_mean)) +
    ...
    theme_bw() +  scale_x_continuous(breaks = round(seq(100*min(data_comp$delta_lifex_index_mean), 100*max(data_comp$delta_lifex_index_mean), by = 0.2),1)) +
    labs(
