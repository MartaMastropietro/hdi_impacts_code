...
result <- foreach(i = (1:4), .combine = 'rbind', .packages = "dplyr") %dopar% {
  
  # Extract group values
  #gdl <- group_keys$gdlcode[i]
  iso <- group_keys$iso3[i]
  scenario <- group_keys$ssp[i]
  yr <- group_keys$year[i]
  
  # Filter data for this group
  subset_data <- data %>% filter( iso3 == iso, ssp == scenario, year == yr)
  
  # Compute summarization
  summarised_data <- subset_data %>%
    filter( iso3 == iso, ssp == scenario, year == yr)%>%
    group_by(gdlcode,iso3,ssp,year)%>%
    summarise(across(-c(model), ~ median(.x, na.rm = TRUE))) 
    summarised_data
  
}

arrow::write_feather(result, file.path(out_dir_lag,
