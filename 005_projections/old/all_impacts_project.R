...
out_dir_pop<-"output/projections/original_comp/area_weight"
...
project_impacts <- function(var_name, climate_projections, data, coefs, out_dir, maps=FALSE){
  ...
  data<-inner_join(data, climate_projections[, c("gdlcode","iso3", "year", "model", "ssp", "delta_clim_proj")])
    ...
    for (sc in unique(data$ssp)){
      for (mod in unique(data$model)){
        if( paste0(mod, "_", sc) %in% data$model_ssp ){
          ...
          plot_map<-inner_join(plot_map, gdl_sub_nations)
...
types<-c("all_extr", "all_extr_controls", "all_extr_autoreg")
# out_variables<-c("gnipc", "leb", "mys")
out_variables<-c( "mys")
