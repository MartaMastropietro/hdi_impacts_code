...
cross_validation_fixest = function (data_dem, f, nmax){
...
model_types<-c("lags_tp_n_8_control", "lags_tp_same_n_control", "lags_extr_alone")
  ...
  for (o in out_variables){
    ...
    for (cl_var in m_modns_cont[-c(1:2)]){
      ...
      if(N==0){
        ...
        results$loocv[id]<-cross_validation_fixest(data_dem=data_dem, f=f, max(N_temp,N_rain, N) )
        ...
        for (spec in specs){
          ...
          mean_cl_formula<-paste0(varns[v_temp], "+", varns[v_rain], "+", 
                                  ...
                                  paste0("lag_",1:N_temp_fix,"_",varns[v_temp], collapse = "+"),"+", 
                                  paste0("lag_",1:N_temp_fix,"_",m_modns[v_temp],'_i_',varns[v_temp], collapse = "+"),"+", 
                                  paste0("lag_",1:N_rain_fix,"_",varns[v_rain], collapse = "+"),"+", 
                                  paste0("lag_",1:N_rain_fix,"_",m_modns[v_rain],'_i_',varns[v_rain], collapse = "+"))
...
write.csv(x=results, file=file.path(out_dir, 'results_oaat_extr_lags.csv'))