...
pop_2020<-terra::rast(file.path(pop_data_dir,"gpw_v4_population_density_rev11_2020_30_sec.tif"))
pop_2020 <- subst(pop_2020, NA, 0) 
pop_2020 <- raster::raster(pop_2020)
# crs adjust
crs(pop_2020)  = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"
#plot(pop_2020)



### Weight by pop
file<-file.path(data_dir, paste0("projections_","PEXT","_", "CAN_ESM5", "_", "ssp126", ".RData"))
load(file)
areas=area(all_rast[[1]][[1]]) #they are all the same, lets take the first
plot(areas)
weight = resample(pop_2020, areas, method='bilinear')
plot(weight)
...
### save all dataframes 
write.csv(data_country, file = file.path(data_dir, "projection_data_climate_country_pop_2020_weight_2015_2100.csv"), row.names = FALSE)
