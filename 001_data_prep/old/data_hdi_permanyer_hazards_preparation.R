...
data_dir <- "data/hdi_data/SHDI-Database 1990-2017"
data_out_all <- read_excel(file.path(data_dir,"SHDI-Database.xlsx"))
colnames(data_out_all)<-c("iso3", "country", "year", "gdlcode", "level", "region", "hdi", "health_index", "income_index", "edu_index", 
                          "lifexp", "log_gni_pc","esch", "msch", "pop")
#keep only subnat 
data_out_all<-data_out_all[which(data_out_all$level=="Subnat"), ]
...
data_cl <- read_csv(file.path(data_dir, "data_extremes_gdl_1960_2022.csv"), show_col_types = FALSE)
...
data$RR_YY_mean<-data$RR_YY/12

data<-data %>% mutate(
  TM_YY_2 = TM_YY^2,
  RR_YY_2 = RR_YY^2,
  RR_YY_mean_2=RR_YY_mean^2,
  HWd_DUR_2= HWd_DUR^2 ,
  HWd_EV_2= HWd_EV^2 ,
...
write.csv(data, file.path(out_dir,"data_hdi_permanyer_hazards_1990_2015.csv"), row.names = FALSE)
