...
for (n in 7:9){
  ...
  save_model<-function(m, pattern, o, type, spec, N){
    ...
    write.csv(tab, file= file.path(out_dir,paste0(o, '_',type, '_lagdiff_fix_',spec,'_lagN',as.character(N),'_coeftab.csv')))
    write.csv(coefs, file= file.path(out_dir,paste0(o, '_',type, '_lagdiff_fix_',spec,'_lagN',as.character(N),'_coef.csv')))
    write.csv(cov, file=file.path(out_dir,paste0(o, '_',type, '_lagdiff_fix_',spec,'_lagN',as.character(N),'_cov.csv')))
  }
  ...
  # save 
  save_model(m, pattern, o, type, spec, N)
  ...
  ### all, moderating with extr, + autoreg + controls
  type<-"all_extr_autoreg_controls"
  r=paste0(mean_cl_formula,"+", 
           extr_formula, "+",
           "l(conflict, 0:8) + l(exp_edu, 0:8)", "+",
           # autoreg
  ...
  extr_formula<-paste0( 
                        ...
                        paste0("lag_",1:N,"_",varns[v_wd], collapse = "+"),"+", 
                        paste0("lag_",1:N,"_",m_modns[v_temp],'_i_',varns[v_wd], collapse = "+"),"+",
                        paste0(varns[v_tvar]), "+",
  ...
  extr_formula<-paste0( paste0(varns[v_wd]), "+",
                        ...
                        paste0("lag_",1:N,"_",varns[v_spi], collapse = "+"),"+", 
                        paste0("lag_",1:N,"_",m_modns[v_temp],'_i_',varns[v_spi], collapse = "+")
  )
  ...
  #                       paste0("lag_",1:N,"_",varns[v_hw], collapse = "+"),"+", 
  #                       paste0("lag_",1:N,"_",m_modns[v_temp],'_i_',varns[v_hw], collapse = "+"),"+",
  #                       paste0(varns[v_spi]), "+",
  #                       paste0(m_modns[v_spi], "_i_", varns[v_spi]),"+", 
  #                       paste0("lag_",1:N,"_",varns[v_spi], collapse = "+"),"+", 
  #                       paste0("lag_",1:N,"_",m_modns[v_spi],'_i_',varns[v_spi], collapse = "+")
  ...
  r <- "l(diff_TM, 0:1) + l(TM_mean_i_diff_TM,0:1) + diff_RR + RR_mean_i_diff_RR +  
...
### each complete mod with/without control and autoreg 
### model specification with mean controls same var
...
### each complete mod with/without control and autoreg 
### model specification with first differences and moderating by income of past year 
...
### all, moderating with adap:cl
type<-"all_extr"
r=paste0(mean_cl_formula,"+", mean_cl_adap_formula,"+",
         extr_formula, "+", extr_adap_formula
)
f= as.formula(paste( o, "~", r, "|" ,i ))
m = fixest::feols(f, data , panel.id = pan_id)
summary(m)
...
### all, moderating with adap:cl, + controls
type<-"all_extr_controls"
r=paste0(mean_cl_formula,"+", mean_cl_adap_formula,"+",
         extr_formula, "+", extr_adap_formula, "+",
         "l(conflict, 0:8) + l(exp_edu, 0:8)"
...
### all,moderating with adap:cl, + autoreg
type<-"all_extr_autoreg"
r=paste0(mean_cl_formula,"+", mean_cl_adap_formula,"+",
         extr_formula, "+", extr_adap_formula, "+",
         # autoreg
...
### all, moderating with adap:cl, + autoreg + controls
type<-"all_extr_autoreg_controls"
r=paste0(mean_cl_formula,"+", mean_cl_adap_formula,"+",
         extr_formula, "+", extr_adap_formula, "+",
         "l(conflict, 0:8) + l(exp_edu, 0:8)", "+",
