...
### just accounting for significant variables
for (r in unique(data$gdlcode)){
  ...
  rx<-mean(data$diff_WD[data$gdlcode == r], na.rm = TRUE)
  pext<-mean(data$diff_PEXT[data$gdlcode == r], na.rm = TRUE)
...
for (r in unique(data$gdlcode)){
  ...
  wd<-mean(data$SPI[data$gdlcode == r], na.rm = TRUE)
...
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- "diff_TM + TM_i_diff_TM + diff_RR + RR_i_diff_RR + diff_TVAR + TM_i_diff_TVAR + diff_WD + TM_i_diff_WD + diff_SPI + TM_i_diff_SPI + diff_HW + TM_i_diff_HW"
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
...
i <- "gdlcode + year"
r <- "diff_TM + TM_i_diff_TM + diff_RR + RR_i_diff_RR + diff_TVAR +TVAR_i_diff_TVAR+  diff_WD + TM_i_diff_WD + diff_RX + RX_i_diff_RX "
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
...
for (r in unique(data$gdlcode)){
  ...
  rx<-mean(data$diff_RX[data$gdlcode == r], na.rm = TRUE)
  wd<-mean(data$diff_WD[data$gdlcode == r], na.rm = TRUE)
  tvar<-mean(data$diff_TVAR[data$gdlcode == r], na.rm = TRUE)
...
for (r in unique(data$gdlcode)){
  ...
  # wd damages
  mean_damages[mean_damages$gdlcode==r, c("wd_diff_mean_damages") ]<-
    wd*0 + wd*tm_contr*coefs["TM_i_diff_WD"] 
  
  # rx damages
  mean_damages[mean_damages$gdlcode==r, c("rx_diff_mean_damages") ]<-
    rx*0 + rx*tm_contr*coefs["TM_i_diff_RX"] 
...
g<-ggplot(mean_damages)+geom_sf( aes(fill=all_mean_damages))+theme_bw()+  scale_fill_gradient2() + 
  labs(fill=paste0(o, " change")) + ggtitle("Marginal effect of extremes") + theme(plot.title = element_text(hjust = 0.5), text = element_text(size = 20))
ggsave(filename=file.path(out_dir, paste0(o,"_complex_mod_regional_mean_damages_only_sign.png")), g, width=18, height=15)
...
for (r in unique(data$gdlcode)){
  ...
  pext<-mean(data$diff_PEXT[data$gdlcode == r], na.rm = TRUE)
  wd<-mean(data$diff_WD[data$gdlcode == r], na.rm = TRUE)
  spi<-mean(data$diff_SPI[data$gdlcode == r], na.rm = TRUE)
...
ggpubr::ggarrange(g2,g3,g4, ncol=1, nrow=3,  common.legend=TRUE, legend="right") %>%
...
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- "diff_TM + TM_i_diff_TM + diff_RR + RR_i_diff_RR + diff_RX +  RR_i_diff_RX + diff_PEXT + TM_i_diff_PEXT + SPI + SPI_2 + lag_log_gni_pc_i_diff_TM + lag_log_gni_pc_i_diff_RR + lag_log_gni_pc_i_diff_PEXT + lag_log_gni_pc_i_diff_RX + lag_log_gni_pc_i_SPI + lag_log_gni_pc_i_SPI_2"
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
