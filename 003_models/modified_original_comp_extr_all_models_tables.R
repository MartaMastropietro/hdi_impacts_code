...
if(!dir.exists(out_dir)){dir.create(out_dir)}
out_dir<-"output/models/original_components_tp_extr_all_models_performances/mean_controls"
if(!dir.exists(out_dir)){dir.create(out_dir)}
...
varns_lev=c('TM','RR', "TVAR", "HW", "RX", "PEXT", "WD", "SPI", "SPEI", "PET")
...
for (i in 1:length(varns)){
  data[paste(modns[i],'_i_',varns_lev[i],sep='')] <- data[modns[i]] * data[varns_lev[i]]
} 
...
for (i in 1:length(varns)){ # 
  data[paste("TM_mean",'_i_',varns_lev[i],sep='')] <- data["TM_mean"] * data[varns_lev[i]]
} 

for (i in 1:length(varns)){ # 
  data[paste("RR_mean",'_i_',varns_lev[i],sep='')] <- data["RR_mean"] * data[varns_lev[i]]
} 

for (i in 1:length(varns)){ # 
  data[paste("TM_mean",'_i_',modns[i],sep='')] <- data["TM_mean"] * data[varns[i]]
} 

for (i in 1:length(varns)){ # 
  data[paste("RR_mean",'_i_',modns[i],sep='')] <- data["RR_mean"] * data[varns[i]]
} 
...
# diff 
i <- "gdlcode"
r <- "diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR "
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
...
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- " diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_RR + lag_1_RR_mean_i_diff_RR  "
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
...
for (o in out_vars){
  for (ext in extr_variables){
    for (i in indeces_new){
      ...
      ###
      r <- paste0("TM + TM_2 + RR + RR_2 + ", ext, " + ", paste0(ext,"_2"), "+", "TM_mean_i_", ext)
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      
      ###
      r <- paste0("TM + TM_2 + RR + RR_2 + ", ext, " + ", paste0(ext,"_2"), "+", "RR_mean_i_", ext)
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR " )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR  + ", paste0("diff_",ext) )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0(ext,"_mean_i_","diff_", ext ))
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0("diff_",ext) ,"+", "TM_mean_i_","diff_", ext )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0("diff_",ext) ,"+", "RR_mean_i_","diff_", ext )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0(paste0("diff_",ext), " + ", paste0(ext,"_mean_i_","diff_", ext ))
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0(ext) )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0(ext), " + ", paste0(ext,"_2"))
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0(ext), " + ", paste0(ext,"_2"), "+", "TM_mean_i_", ext )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      ###
      r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR +
      lag_1_diff_TM + lag_1_TM_mean_i_diff_TM + lag_1_diff_RR + lag_1_RR_mean_i_diff_RR + ", paste0(ext), " + ", paste0(ext,"_2"), "+", "RR_mean_i_", ext )
      f <- as.formula(paste( o, "~", r, "|" ,i ))
      m <- fixest::feols(f, data, panel.id=pan_id)
      models[[length(models)+1]]<-m
      ...
      #  
      #  ###
      #  r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0(ext,"_mean_i_","diff_", ext ), 
      #              "+ lag_log_gni_pc_i_diff_TM","+ lag_log_gni_pc_i_diff_RR" )
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      #  
      #  r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0("TM_mean_i_","diff_", ext ), 
      #              "+ lag_log_gni_pc_i_diff_TM","+ lag_log_gni_pc_i_diff_RR" )
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      #  
      #  r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0("RR_mean_i_","diff_", ext ), 
      #              "+ lag_log_gni_pc_i_diff_TM","+ lag_log_gni_pc_i_diff_RR" )
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      ...
      #  
      #  ###
      #  r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0("TM_mean","_i_","diff_", ext ), 
      #              "+ lag_log_gni_pc_i_diff_TM","+ lag_log_gni_pc_i_diff_RR","+ ", paste0("lag_log_gni_pc","_i_","diff_", ext ))
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      #  
      #  ###
      #  r <- paste0("diff_TM + TM_mean_i_diff_TM + diff_RR + RR_mean_i_diff_RR + ", paste0("diff_",ext), " + ", paste0("RR_mean","_i_","diff_", ext ), 
      #              "+ lag_log_gni_pc_i_diff_TM","+ lag_log_gni_pc_i_diff_RR","+ ", paste0("lag_log_gni_pc","_i_","diff_", ext ))
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      #  
      #  ###
      #  r <- paste0("TM + TM_2 + RR + RR_2 +lag_log_gni_pc_i_TM + lag_log_gni_pc_i_TM_2 + lag_log_gni_pc_i_RR + lag_log_gni_pc_i_RR_2 +", paste0("diff_",ext), " + ", paste0(ext,"_mean_i_","diff_", ext ), " + ", 
      ...
      #  
      #  ###
      #  r <- paste0("TM + TM_2 + RR + RR_2 +lag_log_gni_pc_i_TM + lag_log_gni_pc_i_TM_2 + lag_log_gni_pc_i_RR + lag_log_gni_pc_i_RR_2 + ", paste0("diff_",ext), " + ", paste0("TM_mean","_i_","diff_", ext ), " + ", 
      #              paste0("lag_log_gni_pc","_i_","diff_", ext ))
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
      #  
      #  ###
      #  r <- paste0("TM + TM_2 + RR + RR_2 +lag_log_gni_pc_i_TM + lag_log_gni_pc_i_TM_2 + lag_log_gni_pc_i_RR + lag_log_gni_pc_i_RR_2 +", paste0("diff_",ext), " + ", paste0("RR_mean","_i_","diff_", ext ), " + ", 
      #              paste0("lag_log_gni_pc","_i_","diff_", ext ))
      #  f <- as.formula(paste( o, "~", r, "|" ,i ))
      #  m <- fixest::feols(f, data, panel.id=pan_id)
      #  models[[length(models)+1]]<-m
