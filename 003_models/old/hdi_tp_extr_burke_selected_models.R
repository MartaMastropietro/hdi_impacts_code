#### with final selected model(s) 

### todo
### with / without mean climate controls 
### change data to area weight, save results with / without adap and with/without cl controls 
...
out_dir<-"output/hdi_tp_extr_burke_selected_models_maps"
...
my_data <- data[, c("TM", "RR", "WD", "SPI", "PET", "TVAR")]

jpeg(file=file.path(out_dir, "chart_correlation.jpeg"))
...
# test adding interaction spi tm
o <- "diff_hdi"
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- " TM + TM_2 + RR +RR_2+ WD_2 + SPI + PET + PET_2 + TVAR + TVAR_2+ SPI:TM+
...
o <- "diff_hdi"
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- " TM + TM_2 + RR +RR_2+ WD_2 + SPI + PET + PET_2 + TVAR + TVAR_2+
      lag_log_gni_pc:TM + lag_log_gni_pc:TM_2 + lag_log_gni_pc:RR  + lag_log_gni_pc:RR_2+
       lag_log_gni_pc:WD_2  +
      lag_log_gni_pc:PET + lag_log_gni_pc:PET_2 +lag_log_gni_pc:SPI + lag_log_gni_pc:TVAR + lag_log_gni_pc:TVAR_2 "
f <- as.formula(paste( o, "~", r, "|" ,i ))
...
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- " TM + TM_2 + RR +RR_2+ WD_2 + SPI + PET + PET_2 + TVAR + TVAR_2+
      lag_hdi:TM + lag_hdi:TM_2 + lag_hdi:RR  + lag_hdi:RR_2+
       lag_hdi:WD_2  +
      lag_hdi:PET + lag_hdi:PET_2 +lag_hdi:SPI + lag_hdi:TVAR + lag_hdi:TVAR_2 "
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
summary(m)
summary(m, vcov = ~ iso3)
...
o <- "diff_hdi"
i <- "gdlcode + year + iso3[year] +iso3[year^2]"
r <- " WD_2 + SPI + PET + PET_2 + TVAR + TVAR_2+
       lag_log_gni_pc:WD_2 +
      lag_log_gni_pc:PET + lag_log_gni_pc:PET_2 +lag_log_gni_pc:SPI + lag_log_gni_pc:TVAR + lag_log_gni_pc:TVAR_2 "
f <- as.formula(paste( o, "~", r, "|" ,i ))
m <- fixest::feols(f, data, panel.id=pan_id)
...
modelsummary(models, estimate = "{estimate}{stars}", statistic = NULL, gof_omit = c("Std.Errors"), fmt = fmt_sprintf("%.5e"), output=file.path(out_dir, "hdi_tp_extr_burke_selected_models.html"))
modelsummary(models, vcov=lapply(models, FUN=fvcov_dk), estimate = "{estimate}{stars}", statistic = NULL, gof_omit = c("Std.Errors"), fmt = fmt_sprintf("%.5e"), output=file.path(out_dir, "hdi_tp_extr_burke_selected_models_dk.html"))
modelsummary(models, vcov=lapply(models, FUN=fvcov_iso), estimate = "{estimate}{stars}", statistic = NULL, gof_omit = c("Std.Errors"), fmt = fmt_sprintf("%.5e"), output=file.path(out_dir, "hdi_tp_extr_burke_selected_models_iso3.html"))

################################################################################
### plot of climate controls models, adap

out_dir<-"output/hdi_tp_extr_burke_selected_models_maps/climate_controls_adap_model"
if(!dir.exists(out_dir)){dir.create(out_dir)}
...
for (r in unique(data$gdlcode)){
  ...
  tvar<-mean(data$TVAR[data$gdlcode == r], na.rm = TRUE)
  ...
  tvar_plus1sd<-mean(data$TVAR[data$gdlcode == r], na.rm = TRUE)+sd(data$TVAR[data$gdlcode == r], na.rm = TRUE)
  
  # wd damages
  mean_damages[mean_damages$gdlcode==r, c("wd_mean_damages") ]<-
  ...
  # tvar damages
  mean_damages[mean_damages$gdlcode==r, c("tvar_mean_damages") ]<-
    (tvar_plus1sd*coefs["TVAR"] + (tvar_plus1sd^2)*coefs["TVAR_2"] +
       tvar_plus1sd*laghdi*coefs["TVAR:lag_log_gni_pc"] +(tvar_plus1sd^2)*laghdi*coefs["TVAR_2:lag_log_gni_pc"]) -
    (tvar*coefs["TVAR"] + (tvar^2)*coefs["TVAR_2"] +
       tvar*laghdi*coefs["TVAR:lag_log_gni_pc"] +(tvar^2)*laghdi*coefs["TVAR_2:lag_log_gni_pc"]) 
...
g1<-ggplot(mean_damages)+geom_sf( aes(fill=wd_mean_damages))+theme_bw()+  scale_fill_gradient2() + 
...
g2<-ggplot(mean_damages)+geom_sf( aes(fill=pet_mean_damages))+theme_bw()+ scale_fill_gradient2() +
...
g3<-ggplot(mean_damages)+geom_sf( aes(fill=spi_mean_damages))+theme_bw() + scale_fill_gradient2() +
...
g4<-ggplot(mean_damages)+geom_sf( aes(fill=tvar_mean_damages))+theme_bw() + scale_fill_gradient2() +
  labs(fill="HDI change (%)") + ggtitle("Marginal effect of TVAR")+ theme(plot.title = element_text(hjust = 0.5), text = element_text(size = 24))
ggsave(filename=file.path(out_dir, paste0(o,"_burke_tvar_mean_plus1sd_damages.png")), g4, width=15, height=12)


g5<-ggplot(mean_damages)+geom_sf( aes(fill=all_mean_damages))+theme_bw() + scale_fill_gradient2() +
  labs(fill="HDI change (%)") + ggtitle("Sum of marginal effects for one sd increase of WD, SPI, PET, TVAR")+ theme(plot.title = element_text(hjust = 0.5), text = element_text(size = 24))
ggsave(filename=file.path(out_dir, paste0(o,"_burke_all_mean_plus1sd_damages.png")), g5, width=15, height=12)

# all in one
ggpubr::ggarrange(g1,g2,g3,g4, ncol=1, nrow=4,  common.legend=TRUE, legend="right") %>%
  ggpubr::ggexport(filename = file.path(out_dir,paste0("diff_hdi_burke_extremes_mean_plus1sd_damages.png")), width =700,
                   height = 2400)

################################################################################
### lets increase t of 1 sd, how much sds are the others increasing 
...
mean_sd_TVAR<-mean(unique(data$TVAR_sd))
...
### tvar
names<-c("TVAR", "TVAR:lag_log_gni_pc", "TVAR_2", "TVAR_2:lag_log_gni_pc")
coefficients <- coef(m)[names] # t coefs
cov_matrix <- vcov(m, )[names,names] # t cov

# Generate a sequence of temperature values
values <- seq(min(data$TVAR,  na.rm = TRUE), max(data$TVAR, na.rm = TRUE), length.out = 1000)
...
g_tvar<-ggplot(damages)+
  ...
  geom_ribbon(aes(x=values, ymin=lower_ci_2, ymax=upper_ci_2, fill='High \n(around 18434$)'), alpha=0.2)+
  theme_bw()+theme( text = element_text(size = 34))+
  xlab("Temperature variability (TVAR)")+ylab("HDI change (%)")+scale_fill_brewer(name="Income Per \nCapita Level", breaks=c('Low \n(around 2898$)', 'Median \n(around 8010$)', 'High \n(around 18434$)'), palette="Set1")
...
### all together 
ggpubr::ggarrange(g_wd, g_pet, g_spi, g_tvar,  nrow=1, ncol=4, common.legend = TRUE, legend="right") %>%
  ggpubr::ggexport(filename = file.path(out_dir,paste0("diff_hdi_burke_extremes_by_income_damages.png")), width = 2300,
                   height = 500)
...
tvar_sd<-mean(aggregate(data$TVAR, list(data$gdlcode), FUN=sd, na.rm=TRUE)$x, na.rm=TRUE )
...
tvar2_sd<-mean(aggregate(data$TVAR_2, list(data$gdlcode), FUN=sd, na.rm=TRUE)$x, na.rm=TRUE )
...
tvar_adap_sd<-mean(aggregate(data$TVAR*data$lag_log_gni_pc, list(data$gdlcode), FUN=sd, na.rm=TRUE)$x, na.rm=TRUE )
...
tvar2_adap_sd<-mean(aggregate(data$TVAR_2*data$lag_log_gni_pc, list(data$gdlcode), FUN=sd, na.rm=TRUE)$x, na.rm=TRUE )

std_devs <- c("TM" = tm_sd, "TM_2"=tm2_sd, "RR"=rr_sd, "RR_2"=rr2_sd, 
              "SPI"= spi_sd, "WD"=wd_sd, "WD_2"=wd2_sd, "PET"=pet_sd, "PET_2"=pet2_sd,
              "TVAR" = tvar_sd, "TVAR_2"=tvar2_sd,
              ...
              "TVAR:lag_log_gni_pc" = tvar_adap_sd, "TVAR_2:lag_log_gni_pc"=tvar2_adap_sd
)
coef_data$variable
...
coef_data$variable <- factor(coef_data$variable, levels=c("TM",  "TM:lag_log_gni_pc" ,
                                                          ...
                                                          "TVAR",  "TVAR:lag_log_gni_pc",
                                                          "TVAR_2",  "TVAR_2:lag_log_gni_pc"))
...
ggsave(filename=file.path(out_dir, paste0("components_coefs_scaled_comparison.png")), g, width=30, height=15)
...
### what if we only have small model


### reduced model informing by correlations 
 o <- "diff_hdi"
 i <- "gdlcode + year + iso3[year] +iso3[year^2]"
 r <- " PET + PET_2 + WD_2  + SPI + TVAR + TVAR_2+
       lag_log_gni_pc:PET+ lag_log_gni_pc:PET_2 + 
        lag_log_gni_pc:WD_2  +
       lag_log_gni_pc:TVAR + lag_log_gni_pc:TVAR_2+ lag_log_gni_pc:SPI "
 f <- as.formula(paste( o, "~", r, "|" ,i ))
 m <- fixest::feols(f, data, panel.id=pan_id)
 summary(m)
 summary(m, vcov = ~ iso3)
 ...
 out_dir<-"output/hdi_tp_extr_burke_selected_models_maps/only_extremes_adap_model"
 if(!dir.exists(out_dir)){dir.create(out_dir)}
 ...
 levels_var<-c(
   ...
   "TVAR_2",  "TVAR_2:lag_log_gni_pc",
   "SPI",  "SPI:lag_log_gni_pc"  )
 coef_data$variable <- factor(coef_data$variable, levels=levels_var)
 ...
 g<-ggplot(coef_data, aes(x = variable, y = estimate_rescaled, color = model)) +
   ...
   theme(legend.position = "right",axis.text.x = element_text(angle = 90),text = element_text(size = 40)) 
